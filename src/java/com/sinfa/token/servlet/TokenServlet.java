...
public class TokenServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if ("crear".equals(action)) {
                ...
                confirmarEntrega1(request, response, usuario);
            } else if ("confirmar2".equals(action)) {
                confirmarEntrega2(request, response, usuario);
            } else {
    ...
    // ================= IMPLEMENTADOR ADMIN =================
    else if ("IMPLEMENTADOR_ADMIN".equalsIgnoreCase(perfil)) {

        System.out.println("ROL: IMPLEMENTADOR ADMIN");
        System.out.println("FILTRO CORRECTO: esttokcon IS NULL (PENDIENTE 1)");

        tokens = tokenDAO.listarParaImplementadorAdmin();
        // ✔ SOLO esttokcon IS NULL
    ...
    // ================= IMPLEMENTADOR NO ADMIN =================
        ...
        tokens = tokenDAO.listarParaImplementadorNoAdmin();
        // ✔ esttokcon = 1 (Pend 2)
        // ✔ esttokcon = 2 (Completos)
    ...
    private void crearToken(HttpServletRequest request, HttpServletResponse response, Usuario usuario)
        ...
        token.setFecaccion(Date.valueOf(fechaAccion));
    ...
    private void actualizarTokenAdmin(HttpServletRequest request, HttpServletResponse response, Usuario usuario)
        ...
        private void confirmarEntrega1(HttpServletRequest request, HttpServletResponse response, Usuario usuario)
            throws ServletException, IOException {
        ...
        token.setFecentcon(Date.valueOf(fechaEntrega));
        token.setTxtobscon(observaciones);
        token.setDocSustentoEntrega(nombreArchivo);
        token.setUsumod(usuario.getIdUsuario());

        tokenDAO.actualizarConfirmacion1(token);
    ...
    private void confirmarEntrega2(HttpServletRequest request, HttpServletResponse response, Usuario usuario)
            throws ServletException, IOException {
        ...
        token.setFecentcon2(Date.valueOf(fechaEntrega));
        token.setTxtobscon2(observaciones);
        token.setDocSustentoFinal(nombreArchivo);
        token.setUsumod(usuario.getIdUsuario());
        
        boolean ok = tokenDAO.actualizarConfirmacion2(token);
    ...
    private String getEstadoTexto(int estado) {
        switch (estado) {
            ...
            case 4: return "ENTREGADO A UNIDAD ANTERIOR";
            default: return "N/A";
        }
    }
}